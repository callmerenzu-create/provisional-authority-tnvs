...
const WordPressPopularPosts = (() => {
    ...
    const ajax = (method, url, params, callback, additional_headers) => {
        ...
        }).then(data => callback(data)).catch(error => console.error('Fetch error:', error))
    };
    const theme = (wpp_list) => {
        ...
        const wpp_list_sr = wpp_list.attachShadow({
            mode: "open"
        });
...
(() => {
    ...
    if (post_id) {
        ...
        if (do_request) {
            WordPressPopularPosts.post(`${wpp_params.apiUrl }/v2/views/${ post_id }`, `sampling=${wpp_params.sampling }&sampling_rate=${wpp_params.samplingRate }`, (response) => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function fetchWidget(widget_placeholder) {
        ...
        WordPressPopularPosts.post(`${wpp_params.apiUrl }/v2/widget?is_single=${wpp_params.postId }${wpp_params.lang?`&lang=${wpp_params.lang }`:''}`, params, response => renderWidget(response, widget_placeholder), {
    ...
    function renderWidget(response, widget_placeholder) {
        ...
        const event = new Event('wpp-onload', {
            bubbles: true,
            cancelable: false
        });
